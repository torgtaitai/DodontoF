...
　　　／變調表　WRONG／戰場表　BATTLE／異形表　MT／隨機特技決定表　RTT
...
　　／夜行列車場景表　HY／病院場景表　HO／龍動場景表　HR／密室場景表　CHAMBER／催眠場景表　HS
	／基地突破情景表　BASE／(自創)遊樂場情景表　OMG／文化祭 SH
・D66骰表
INFO_MESSAGE_TEXT
  end
  ...
  def sinobigami_fumble_table(string)
    ...
    when /KFT/
      ...
      table = [
               ...
               '油断した！　術の制御に失敗し、好きな【生命力】を１点失う。',
               '敵の陰謀か？　罠にかかり、ランダムに選んだ變調１つを受ける。變調は、變調表で決定すること。',
               'ふう。危ないところだった。特に何も起こらない。',
              ]
    end
  ...
  def sinobigami_wrong_table(string)
    ...
    when /KWT/
      ...
      table = [
               ...
               '行方不明:その戰闘終了後、メインフェイズ中に行動不可。１サイクルの終了時に、《経済力》で成功すると無効化される。',
